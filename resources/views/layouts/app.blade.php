...
<head>
    ...
    <style>
        ...
        .brand { border-bottom: 1px solid rgba(255,255,255,.16); padding: 1.15rem 1.25rem; font-weight: 700; line-height: 1.3; color: #fff; text-decoration: none; display: block; }
...
<body>
    <aside class="sidebar">
        <a class="brand" href="{{ route('dashboard') }}"><i class="bi bi-mortarboard-fill me-1"></i><span>ĐẠI HỌC THỦY LỢI</span><small>CLUB SYSTEM</small></a>
        <nav class="pt-3">
            <a class="nav-link {{ $routeName === 'dashboard' ? 'active' : '' }}" href="{{ route('dashboard') }}"><i class="bi bi-grid-1x2"></i><span>Dashboard</span></a>
            <a class="nav-link {{ str_starts_with($routeName, 'students.') ? 'active' : '' }}" href="{{ route('students.index') }}"><i class="bi bi-person-vcard"></i><span>Sinh viên</span></a>
            ...
            <a class="nav-link {{ str_starts_with($routeName, 'membership-requests.') ? 'active' : '' }}" href="{{ route('membership-requests.index') }}"><i class="bi bi-person-check"></i><span>Duyệt yêu cầu CLB</span></a>
            ...
            <a class="nav-link {{ str_starts_with($routeName, 'activity-point-rules.') ? 'active' : '' }}" href="{{ route('activity-point-rules.index') }}"><i class="bi bi-list-stars"></i><span>Quy tắc điểm</span></a>
            <a class="nav-link {{ str_starts_with($routeName, 'student-points.') ? 'active' : '' }}" href="{{ route('student-points.index') }}"><i class="bi bi-award"></i><span>Điểm sinh viên</span></a>
